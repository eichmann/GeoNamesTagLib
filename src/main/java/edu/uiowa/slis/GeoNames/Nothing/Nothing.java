...
public class Nothing extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static Nothing currentInstance = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			NothingIterator theNothingIterator = (NothingIterator) findAncestorWithClass(this, NothingIterator.class);

			if (theNothingIterator != null) {
				subjectURI = theNothingIterator.getSubjectURI();
				label = theNothingIterator.getLabel();
			}

			if (theNothingIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Nothing doStartTag", e);
			throw new JspTagException("Exception raised in Nothing doStartTag");
		} finally {
			freeConnection();
		}
