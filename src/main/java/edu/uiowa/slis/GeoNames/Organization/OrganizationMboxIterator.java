...
public class OrganizationMboxIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationMboxIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationMboxIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/mbox> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				mbox = sol.get("?s").toString();
