...
public class OrganizationYahooChatIDIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationYahooChatIDIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationYahooChatIDIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/yahooChatID> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				yahooChatID = sol.get("?s").toString();
