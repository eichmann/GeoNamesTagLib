...
public class OrganizationWeblogIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationWeblogIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationWeblogIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/weblog> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				weblog = sol.get("?s").toString();
