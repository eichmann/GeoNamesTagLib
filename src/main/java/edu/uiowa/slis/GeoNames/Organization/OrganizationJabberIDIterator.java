...
public class OrganizationJabberIDIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationJabberIDIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationJabberIDIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/jabberID> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				jabberID = sol.get("?s").toString();
