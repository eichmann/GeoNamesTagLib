...
public class OrganizationOpenidIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationOpenidIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationOpenidIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/openid> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				openid = sol.get("?s").toString();
