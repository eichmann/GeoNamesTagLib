...
public class OrganizationIcqChatIDIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationIcqChatIDIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationIcqChatIDIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/icqChatID> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				icqChatID = sol.get("?s").toString();
