...
public class OrganizationAccountIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static OrganizationAccountIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OrganizationAccountIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/account> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				account = sol.get("?s").toString();
