...
public class Group extends edu.uiowa.slis.GeoNames.TagLibSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (this.getParent() instanceof edu.uiowa.slis.GeoNames.Agent.AgentMemberInverseIterator) {
				subjectURI = ((edu.uiowa.slis.GeoNames.Agent.AgentMemberInverseIterator)this.getParent()).getMemberInverse();
			}
			...
			if (theGroupIterator == null && subjectURI == null) {
				...
				+ " SELECT ?label ?foafName ?schemaName ?rdfValue  ?member ?openid ?icqChatID ?msnChatID ?weblog ?account ?interest ?mbox_sha1sum ?made ?tipjar ?age ?aimChatID ?gender ?yahooChatID ?holdsAccount ?jabberID ?topic_interest ?skypeID ?birthday ?status ?mbox where {"
				+ "  OPTIONAL { <" + subjectURI + "> rdfs:label ?label } "
				+ "  OPTIONAL { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/name> ?foafName } "
				+ "  OPTIONAL { <" + subjectURI + "> <http://schema.org/name> ?schemaName } "
				+ "  OPTIONAL { <" + subjectURI + "> <http://www.w3.org/1999/02/22-rdf-syntax-ns#value> ?rdfValue } "
				+ "  OPTIONAL { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/member> ?member } "
