...
public class GeonamesFeature extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static GeonamesFeature currentInstance = null;
	private static final Log log = LogFactory.getLog(GeonamesFeature.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			GeonamesFeatureIterator theGeonamesFeatureIterator = (GeonamesFeatureIterator) findAncestorWithClass(this, GeonamesFeatureIterator.class);

			if (theGeonamesFeatureIterator != null) {
				subjectURI = theGeonamesFeatureIterator.getSubjectURI();
				label = theGeonamesFeatureIterator.getLabel();
			}

			if (theGeonamesFeatureIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in GeonamesFeature doStartTag", e);
			throw new JspTagException("Exception raised in GeonamesFeature doStartTag");
		} finally {
			freeConnection();
		}
