...
public class GeonamesFeatureAltIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static GeonamesFeatureAltIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(GeonamesFeatureAltIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2003/01/geo/wgs84_pos#alt> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				alt = sol.get("?s").toString();
