...
public class GeonamesFeatureLocationMapIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static GeonamesFeatureLocationMapIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(GeonamesFeatureLocationMapIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.geonames.org/ontology#locationMap> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				locationMap = sol.get("?s").toString();
