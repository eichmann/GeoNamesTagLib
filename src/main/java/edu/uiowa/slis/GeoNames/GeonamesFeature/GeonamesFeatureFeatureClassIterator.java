...
public class GeonamesFeatureFeatureClassIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static GeonamesFeatureFeatureClassIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(GeonamesFeatureFeatureClassIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.geonames.org/ontology#featureClass> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				featureClass = sol.get("?s").toString();
