...
public class CodeInSchemeIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static CodeInSchemeIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(CodeInSchemeIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2004/02/skos/core#inScheme> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				inScheme = sol.get("?s").toString();
