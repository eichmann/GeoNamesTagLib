...
public class CodeHistoryNoteIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static CodeHistoryNoteIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(CodeHistoryNoteIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2004/02/skos/core#historyNote> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				historyNote = sol.get("?s").toString();
				firstInstance = true;
	...
	public  void setHistoryNote(String theHistoryNote) {
		historyNote = theHistoryNote;
	}
