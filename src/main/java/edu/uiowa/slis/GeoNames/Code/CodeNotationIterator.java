...
public class CodeNotationIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static CodeNotationIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(CodeNotationIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://www.w3.org/2004/02/skos/core#notation> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				notation = sol.get("?s").toString();
				firstInstance = true;
	...
	public  void setNotation(String theNotation) {
