...
public class PersonImgIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonImgIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonImgIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/img> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				img = sol.get("?s").toString();
