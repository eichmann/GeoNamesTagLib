...
public class PersonWorkInfoHomepageIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonWorkInfoHomepageIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonWorkInfoHomepageIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/workInfoHomepage> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				workInfoHomepage = sol.get("?s").toString();
