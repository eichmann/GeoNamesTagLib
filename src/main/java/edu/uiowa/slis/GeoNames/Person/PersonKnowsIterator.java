...
public class PersonKnowsIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonKnowsIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonKnowsIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/knows> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				knows = sol.get("?s").toString();
