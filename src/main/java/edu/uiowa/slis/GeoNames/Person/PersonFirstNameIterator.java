...
public class PersonFirstNameIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonFirstNameIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonFirstNameIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/firstName> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				firstName = sol.get("?s").toString();
