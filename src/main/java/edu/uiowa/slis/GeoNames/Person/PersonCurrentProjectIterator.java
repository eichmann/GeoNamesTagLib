...
public class PersonCurrentProjectIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonCurrentProjectIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonCurrentProjectIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/currentProject> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				currentProject = sol.get("?s").toString();
