...
public class PersonPastProjectIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonPastProjectIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonPastProjectIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/pastProject> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				pastProject = sol.get("?s").toString();
