...
public class PersonWorkplaceHomepageIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonWorkplaceHomepageIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonWorkplaceHomepageIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/workplaceHomepage> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				workplaceHomepage = sol.get("?s").toString();
