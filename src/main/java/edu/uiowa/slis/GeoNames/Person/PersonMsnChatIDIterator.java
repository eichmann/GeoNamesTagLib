...
public class PersonMsnChatIDIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonMsnChatIDIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonMsnChatIDIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/msnChatID> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				msnChatID = sol.get("?s").toString();
