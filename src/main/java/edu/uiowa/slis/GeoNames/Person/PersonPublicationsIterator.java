...
public class PersonPublicationsIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonPublicationsIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonPublicationsIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/publications> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				publications = sol.get("?s").toString();
