...
public class PersonGeekcodeIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonGeekcodeIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonGeekcodeIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/geekcode> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				geekcode = sol.get("?s").toString();
