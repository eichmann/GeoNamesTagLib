...
public class PersonSurnameIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonSurnameIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonSurnameIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/surname> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				surname = sol.get("?s").toString();
