...
public class PersonMyersBriggsIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonMyersBriggsIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonMyersBriggsIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/myersBriggs> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				myersBriggs = sol.get("?s").toString();
