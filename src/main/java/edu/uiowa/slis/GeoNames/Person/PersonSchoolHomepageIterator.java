...
public class PersonSchoolHomepageIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonSchoolHomepageIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonSchoolHomepageIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/schoolHomepage> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				schoolHomepage = sol.get("?s").toString();
