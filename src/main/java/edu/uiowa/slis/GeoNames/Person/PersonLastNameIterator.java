...
public class PersonLastNameIterator extends edu.uiowa.slis.GeoNames.TagLibSupport {
	static PersonLastNameIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(PersonLastNameIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://xmlns.com/foaf/0.1/lastName> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				lastName = sol.get("?s").toString();
